...
export async function stashAndCheckout(directory: string, branch: string): Promise<GitStashResult> {
	...
	try {
		...
		if (needsStash) {
			await git.stash(["push", "-m", `gcode: auto-stash before switching to ${branch}`])
		}
...
export async function applyDiffTextToLocal(
	...
	const tmpFile = path.join(os.tmpdir(), `gcode-remote-patch-${Date.now()}.patch`)
...
export async function applyChangesToLocal(
	...
	try {
		...
		const os = await import("node:os")
		const fs = await import("node:fs/promises")
		const tmpFile = path.join(os.tmpdir(), `gcode-patch-${Date.now()}.patch`)
