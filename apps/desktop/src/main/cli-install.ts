 ...
 *
 * macOS packaged:  GCode.app/Contents/Resources/bin/gcode
 * Linux packaged:  <install-dir>/resources/bin/gcode
 * Windows:         <install-dir>/resources/bin/gcode.cmd
 * Dev mode:        <repo>/apps/desktop/resources/bin/gcode
 */
function getCliSourcePath(): string {
	const scriptName = process.platform === "win32" ? "gcode.cmd" : "gcode"
 ...
 * Windows:     %LOCALAPPDATA%\GCode\bin  (added to PATH by the user)
 */
function getCliInstallDir(): string {
	if (process.platform === "win32") {
		const localAppData =
			process.env.LOCALAPPDATA ?? path.join(app.getPath("home"), "AppData", "Local")
		return path.join(localAppData, "GCode", "bin")
	}
	return "/usr/local/bin"
}

/**
 * Returns the full path where the `gcode` symlink/script will be placed.
 */
function getCliInstallPath(): string {
	const name = process.platform === "win32" ? "gcode.cmd" : "gcode"
...
/**
 * Installs the `gcode` CLI command by symlinking it into /usr/local/bin
...
/**
 * Uninstalls the `gcode` CLI command by removing the symlink.
 */
