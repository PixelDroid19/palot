...
export function AboutSettings() {
	...
	useEffect(() => {
		...
		window.gcode.getAppInfo().then((info) => {
			setAppVersion(info.version)
			setIsDev(info.isDev)
		})
		window.gcode.cli.isInstalled().then(setCliInstalled)
	}, [])
	...
	const handleCliInstall = useCallback(async () => {
		...
		setCliError(null)
		try {
			const result = await window.gcode.cli.install()
			if (result.success) {
	...
	const handleCliUninstall = useCallback(async () => {
		...
		setCliError(null)
		try {
			const result = await window.gcode.cli.uninstall()
			if (result.success) {
