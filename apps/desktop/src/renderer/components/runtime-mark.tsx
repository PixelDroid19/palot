 ...
 *
 * Path data matches models.dev official provider logos (same CDN source as
...
/**
 * Anthropic / Claude — official lettermark from models.dev/logos/anthropic.svg
 * Brand accent #D97757 (Anthropic orange).
 */
function ClaudeMark({ size, className, label }: { size: number; className?: string; label: string }) {
	return (
		<SvgShell size={size} className={className} label={label} viewBox="0 0 40 40">
			<path
...
/**
 * OpenAI / Codex — official blossom mark from models.dev/logos/openai.svg
 * (Codex uses the OpenAI product mark).
 */
...
/**
 * OpenCode — official mark from models.dev/logos/opencode.svg
 * (same geometry as opencode.ai brand kit icon).
 */
function OpenCodeMark({
	...
	return (
		<SvgShell size={size} className={className} label={label} viewBox="0 0 24 24">
			<path
...
export const RuntimeMark = memo(function RuntimeMark({
	...
	const failed = animation === "failed" ? "opacity-90 ring-1 ring-red-500/50 rounded-sm" : ""
